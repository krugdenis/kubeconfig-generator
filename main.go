...
func main() {
	...
	var skipIP bool
	var deleteSa bool
	flag.StringVar(&email, "email", "", "Email address (required)")
	flag.StringVar(&clusterServer, "ip", "", "Cluster IP address (optional)")
	flag.StringVar(&clusterRole, "cr", "", "Custom cluster role yaml (optional)")
	flag.BoolVar(&skipIP, "skipIP", false, "Skip providing IP and use the default from the selected context")
	flag.BoolVar(&deleteSa, "delete", false, "Skip creating service account, delete only")
	flag.Parse()
